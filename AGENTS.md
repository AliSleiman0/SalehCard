...
Guidance for Codex (and humans) working in this repo. Read the linked docs
...
> copy from each `.env.example`. `.Codex/memory/` holds the prior session memory; see its
> README to restore it into `~/.Codex`.
...
- End commit messages with the `Co-Authored-By: Codex Opus 4.8 (1M context) <noreply@anthropic.com>` trailer.
