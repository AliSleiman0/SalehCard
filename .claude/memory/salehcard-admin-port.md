...
**Wired to the Go API:** `products` (list/create/edit/delete/bulk + category dropdowns from catalog-distinct facets `/api/admin/products/categories`), `inventory` (code stock, bulk upload w/ dedup, threshold config, code audit, **upload history** via the `upload_batches` collection + `/api/admin/upload-history`), the **dashboard** (now fully real — all KPIs/sparklines/charts/fulfillment/low-stock/system-health), and **settings → admin-users** table (reuses `useUsers({role:'admin'})` + `adaptUser`). **Mock-driven (TODO + ComingSoonNote banner, data in `lib/mock/demo.ts`):** orders, resellers, finance, promos, reviews, remaining settings store-config forms. `CATS`/`admins`/`uploadHistory` mocks deleted from `demo.ts`.
