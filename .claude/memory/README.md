...
This is the **full current memory set** (refreshed 2026-07-08 from the dev machine — 22 files,
not just the original 3). `MEMORY.md` is the index Claude loads each session (one line per memory);
the `salehcard-*.md` files are the individual facts it links to. The restore recipe below copies
all of them, so it stays correct as the set grows — no need to enumerate them here.
